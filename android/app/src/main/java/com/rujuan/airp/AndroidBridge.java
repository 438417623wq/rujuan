...
public class AndroidBridge {
    ...
    public void saveFile(String content, String filename, String mimeType) {
        ...
        showToast(ok ? "文件已保存到 Downloads/AIRP" : "文件保存失败");
    }
    ...
    public void cancelNativeAiStream(String sessionId) {
        ...
        Intent intent = new Intent(context, NativeAiStreamService.class);
        intent.setAction(NativeAiStreamService.ACTION_CANCEL);
        intent.putExtra(NativeAiStreamService.EXTRA_SESSION_ID, sessionId);
        context.startService(intent);
    }
    ...
    public void notifyAiStreaming(String targetPath, String conversationTitle) {
        mainHandler.post(() -> {
            ...
            NotificationCompat.Builder builder = new NotificationCompat.Builder(context, AI_NOTIFICATION_CHANNEL_ID)
                    .setSmallIcon(android.R.drawable.stat_notify_chat)
                    .setContentTitle(normalizeConversationTitle(conversationTitle))
                    .setContentText(context.getString(R.string.notification_ai_generating))
    ...
    public void notifyAiReply(String targetPath, String conversationTitle, String previewText) {
        mainHandler.post(() -> {
            ...
            String preview = normalizePreviewText(previewText);
            NotificationCompat.Builder builder = new NotificationCompat.Builder(context, AI_NOTIFICATION_CHANNEL_ID)
                    .setSmallIcon(android.R.drawable.stat_notify_chat)
                    .setContentTitle(normalizeConversationTitle(conversationTitle))
    ...
    private String defaultBaseUrl(String provider) {
        return switch (provider) {
            ...
            case "google-ai-studio" -> "https://generativelanguage.googleapis.com/v1beta";
    ...
    private boolean saveToDownloads(String content, String filename, String mimeType) {
        ...
        values.put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/AIRP");
    ...
    private String normalizePreviewText(String previewText) {
        if (previewText == null || previewText.isBlank()) {
            return context.getString(R.string.notification_ai_reply_fallback);
        }

        String cleaned = previewText.replaceAll("\\s+", " ").trim();
        if (cleaned.length() > 120) {
            cleaned = cleaned.substring(0, 117) + "...";
        }
        return cleaned.isEmpty() ? context.getString(R.string.notification_ai_reply_fallback) : cleaned;
    }
