...
public class NativeAiStreamService extends Service {
    public static final String ACTION_START = "com.rujuan.airp.action.START_NATIVE_AI_STREAM";
    public static final String ACTION_CANCEL = "com.rujuan.airp.action.CANCEL_NATIVE_AI_STREAM";
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        startForeground(
                ...
                buildOngoingNotification(
                        sessionId,
                        payload.optString("conversationTitle"),
                        getString(R.string.notification_ai_background_connecting),
                        "",
                        true
                )
        );
    ...
    private void handleCancel(String sessionId, int startId) {
        if (sessionId != null && sessionId.equals(activeSessionId)) {
            ...
            JSONObject cancelled = buildBaseState(sessionId, "", "", "");
            putStatus(cancelled, "cancelled");
            putError(cancelled, "");
            NativeAiStreamStore.writeState(this, cancelled);
        }
        stopSelf(startId);
    }
                ...
                try {
                    ...
                    if (cancelRequested.get()) {
                        ...
                        showTerminalNotification(
                                ...
                                fullText,
                                fullText.isBlank()
                                        ? getString(R.string.notification_ai_reply_fallback)
                                        : normalizePreviewText(fullText)
                        );
                    }
                    return;
    ...
    private void showRetryNotification(String sessionId, String conversationTitle, int attempt) {
        NotificationManagerCompat.from(this).notify(
                ...
                buildOngoingNotification(
                        sessionId,
                        conversationTitle,
                        getString(R.string.notification_ai_background_connecting) + " · 重试 " + attempt + "/" + MAX_RETRY_ATTEMPTS,
                        "",
                        true
                )
        );
    ...
    private android.app.Notification buildOngoingNotification(
        ...
        NotificationCompat.Builder builder = createBaseNotification(sessionId, conversationTitle)
                .setContentText(previewText == null || previewText.isBlank() ? contentText : previewText)
                .setStyle(new NotificationCompat.BigTextStyle().bigText(
                        previewText == null || previewText.isBlank() ? contentText : previewText
                ))
                .setOngoing(true)
    ...
    private void updateOngoingNotification(String sessionId, String conversationTitle, String fullText, long startedAt) {
        ...
        NotificationManagerCompat.from(this).notify(
                ...
                buildOngoingNotification(
                        sessionId,
                        conversationTitle,
                        getString(R.string.notification_ai_background_generating) + " · " + elapsed,
                        preview,
                        false
                )
        );
    ...
    private void showTerminalNotification(String sessionId, String conversationTitle, String fullText, String contentText) {
        NotificationCompat.Builder builder = createBaseNotification(sessionId, conversationTitle)
                ...
                .setStyle(new NotificationCompat.BigTextStyle().bigText(
                        fullText == null || fullText.isBlank() ? contentText : normalizePreviewText(fullText)
                ))
                .setAutoCancel(true)
